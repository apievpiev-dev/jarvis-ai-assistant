"""
WebSocket менеджер для Brain Service
Управление WebSocket соединениями для AI обработки
"""
import asyncio
import json
import logging
...
logger = get_logger("brain-websocket-manager")

class WebSocketManager:
    """Менеджер WebSocket соединений для Brain Service"""
    ...
    async def connect(self, websocket: WebSocket):
        ...
        try:
            ...
            logger.info(f"Brain WebSocket client connected. Total connections: {self.stats['active_connections']}")
            
            # Отправка приветственного сообщения
            await self.send_message(websocket, {
                "type": "connection_established",
                "message": "Подключение к Brain Service установлено",
                "session_id": self.connection_info[websocket]["session_id"],
                "server_time": asyncio.get_event_loop().time()
            })
            
        except Exception as e:
            logger.error(f"Failed to connect Brain WebSocket: {e}")
            self.stats["errors"] += 1
            raise
    ...
    async def disconnect(self, websocket: WebSocket):
        ...
        try:
            ...
            logger.info(f"Brain WebSocket client disconnected. Active connections: {self.stats['active_connections']}")
            
        except Exception as e:
            logger.error(f"Error during Brain WebSocket disconnect: {e}")
            self.stats["errors"] += 1
    ...
    async def send_message(self, websocket: WebSocket, message: Dict[str, Any]):
        ...
        try:
            if websocket in self.active_connections:
                ...
                elif message.get("type") == "response_generated":
                ...
                if websocket in self.connection_info:
                    ...
                    if message.get("type") == "command_result":
                        self.connection_info[websocket]["commands_processed"] += 1
                
                logger.debug(f"Brain message sent: {message['type']}")
                
        except Exception as e:
            logger.error(f"Failed to send Brain message: {e}")
            self.stats["errors"] += 1
    ...
    async def broadcast_message(self, message: Dict[str, Any], room: str = None):
        ...
        try:
            ...
            logger.info(f"Brain message broadcasted to {len(connections)} clients")
            
        except Exception as e:
            logger.error(f"Failed to broadcast Brain message: {e}")
            self.stats["errors"] += 1
    ...
    async def join_room(self, websocket: WebSocket, room: str):
        ...
        try:
            if websocket in self.active_connections:
                ...
                logger.info(f"Brain WebSocket client joined room: {room}")
                
        except Exception as e:
            logger.error(f"Failed to join Brain room: {e}")
            self.stats["errors"] += 1
    ...
    async def leave_room(self, websocket: WebSocket, room: str):
        ...
        try:
            if room in self.rooms and websocket in self.rooms[room]:
                ...
                logger.info(f"Brain WebSocket client left room: {room}")
                
        except Exception as e:
            logger.error(f"Failed to leave Brain room: {e}")
            self.stats["errors"] += 1
    ...
    async def handle_message(self, websocket: WebSocket, message: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(f"Brain message handled: {message_type}")
            
        except Exception as e:
            logger.error(f"Failed to handle Brain message: {e}")
            self.stats["errors"] += 1
    ...
    async def get_message(self, websocket: WebSocket) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to get Brain message: {e}")
            return None
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "avg_commands_per_connection": (
                self.stats["commands_processed"] / max(self.stats["total_connections"], 1)
            )
        }
    ...
    async def cleanup_inactive_connections(self, timeout: int = 300):
        ...
        try:
            ...
            for websocket in inactive_connections:
                logger.info("Removing inactive Brain WebSocket connection")
                await self.disconnect(websocket)
            
            if inactive_connections:
                logger.info(f"Cleaned up {len(inactive_connections)} inactive Brain connections")
                
        except Exception as e:
            logger.error(f"Failed to cleanup inactive Brain connections: {e}")
    ...
    async def start_cleanup_task(self, interval: int = 60):
        ...
        while True:
            try:
                ...
                logger.error(f"Brain cleanup task error: {e}")
                await asyncio.sleep(interval)