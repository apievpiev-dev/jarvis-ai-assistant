...
# Скрипт проверки статуса Jarvis AI Assistant
...
# Конфигурация
LOG_FILE="/workspace/jarvis/logs/status.log"
...
check_resource_usage() {
    ...
    log_to_file "INFO: CPU: ${cpu_usage}%, Память: ${memory_usage}%, Диск: ${disk_usage}%"
}
...
check_database() {
    ...
    if docker-compose exec -T postgres pg_isready -U jarvis -d jarvis_db &> /dev/null; then
        ...
        log_to_file "SUCCESS: PostgreSQL доступен, размер: $db_size"
    else
...
check_redis() {
    ...
    if docker-compose exec -T redis redis-cli ping &> /dev/null; then
        ...
        log_to_file "SUCCESS: Redis доступен, память: $redis_memory, ключи: $key_count"
    else
...
check_logs() {
    ...
    if [ -f "docker-compose.yml" ]; then
        ...
        if [ "$error_count" -gt 0 ]; then
            ...
            log_to_file "SUCCESS: Ошибок в логах не найдено"
        fi
    fi
}
...
# Генерация отчета статуса
generate_status_report() {
    log_info "Генерация отчета статуса..."
    
    report_file="/workspace/jarvis/logs/status_report_$(date +%Y%m%d_%H%M%S).txt"
    
    cat > "$report_file" << EOF
Jarvis AI Assistant - Отчет статуса
===================================
...
EOF
    ...
    log_success "Отчет статуса сохранен: $report_file"
}

# Основная функция
main() {
    echo "=========================================="
    echo "  Jarvis AI Assistant - Status Check"
    echo "=========================================="
    echo
    
    log_to_file "INFO: Начало проверки статуса"
    ...
    log_to_file "SUCCESS: Проверка статуса завершена"
    ...
    echo "=========================================="
    echo
    echo "Лог статуса: $LOG_FILE"
    echo "Отчет: /workspace/jarvis/logs/status_report_$(date +%Y%m%d_%H%M%S).txt"
    echo
}
